...
//        create +=  Table_player.FIELD_IDTEAM + " INTEGER PRIMARY KEY AUTOINCREMENT,";
...
//        writeDatiStandard(db);
    ...
    /* Scrive una nuovo giocatore nel db */
    ...
    /* Restituisce tutti i giocatori presenti nel db */

    public ArrayList<Player> getPlayer(){

        ArrayList<Player> player = new ArrayList<Player>();

        String query = "SELECT * FROM " + Table_player.TABLE_NAME;

        Cursor cursor = this.getReadableDatabase().rawQuery(query,null);

        while (cursor.moveToNext()) {

//            int idTeam = cursor.getInt(cursor.getColumnIndex(Table_player.FIELD_IDTEAM));
            int id = cursor.getInt(cursor.getColumnIndex(Table_player.FIELD_ID));
            String name = cursor.getString(cursor.getColumnIndex(Table_player.FIELD_NAME));
            String surname = cursor.getString(cursor.getColumnIndex(Table_player.FIELD_SURNAME));
            Long birthdate = cursor.getLong(cursor.getColumnIndex(Table_player.FIELD_BIRTHDATE));
            int idnation = cursor.getInt(cursor.getColumnIndex(Table_player.FIELD_IDNATION));
            int idrole = cursor.getInt(cursor.getColumnIndex(Table_player.FIELD_IDROLE));
            String telephonenumber = cursor.getString(cursor.getColumnIndex(Table_player.FIELD_TELEPHONENUMBER));
            String email = cursor.getString(cursor.getColumnIndex(Table_player.FIELD_EMAIL));

            Player t = new Player(id,name,surname,birthdate,idnation,idrole,telephonenumber,email);
            player.add(t);
